...
data_dir = osp.normpath(osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/delta/train"))

cls_indexes = sorted(idx2class.keys())
cls_names = [idx2class[cls_idx] for cls_idx in cls_indexes]
lm_model_dir = osp.normpath(osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/delta/models/"))
model_paths = [osp.join(lm_model_dir, f"obj_{obj_id:06d}.ply") for obj_id in cls_indexes]
texture_paths = None

scenes = [i for i in range(0,50)]
xyz_root = osp.normpath(osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/delta/xyz_crop"))

K = np.array([[544.89227, 0, 311.66284], [0, 545.41659, 237.83059], [0, 0, 1]])
                ...
                for anno_i, anno in enumerate(gt_dict[str_im_id]):
                    ...
                    if mask.sum() == 0:  # NOTE: this should be ignored at training phase
                        ...
                        if VIS:
                            # import cv2
                            # print(depth_gl * 1000)
                            # depth_gl = depth_gl * 10000
                            # print(scene_id)
                            # print(int_im_id)
                            # if int_im_id == 27:
                            #     cv2.imwrite('./testabc.png', depth_gl.astype("uint16"))
                            #     return
                            # else:
                            #     continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-gpus", default=False, help="vis")
    parser.add_argument("--config-file", default=False, help="vis")
