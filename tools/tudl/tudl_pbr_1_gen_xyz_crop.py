...
data_dir = osp.normpath(
    osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/tudl/train_real"))

cls_indexes = sorted(id2obj.keys())
cls_names = [id2obj[cls_idx] for cls_idx in cls_indexes]
itodd_model_dir = osp.normpath(
    osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/tudl/models"))
model_paths = [
    osp.join(itodd_model_dir, f"obj_{obj_id:06d}.ply") for obj_id in cls_indexes]
texture_paths = None

scenes = [i for i in range(1, 3 + 1)]
xyz_root = osp.normpath(
    osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/tudl/train_real/xyz_crop"))

K = np.array(
    [[515.0, 0.0, 321.566], [0.0, 515.0, 214.08], [0, 0, 1]])
