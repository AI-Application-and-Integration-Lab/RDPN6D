...
# object info
objects = [str(i) for i in range(1, 30 + 1)]
id2obj = {i: str(i) for i in range(1, 30 + 1)}

data_dir = osp.normpath(
    osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/tless/train_pbr"))

cls_indexes = sorted(id2obj.keys())
cls_names = [id2obj[cls_idx] for cls_idx in cls_indexes]
tless_model_dir = osp.normpath(
    osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/tless/models_reconst"))
model_paths = [
    osp.join(tless_model_dir, f"obj_{obj_id:06d}.ply") for obj_id in cls_indexes]
texture_paths = None

scenes = [i for i in range(0, 49 + 1)]
xyz_root = osp.normpath(
    osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/tless/train_pbr/xyz_crop"))
    ...
    def main(self):
        ...
        for scene_id in tqdm(sel_scene_ids, postfix=f"{split}_{scene}"):
            ...
            for str_im_id in tqdm(gt_dict, postfix=f"{scene_id}"):
                ...
                K = np.array(cam_dict[str_im_id]["cam_K"]).reshape((3,3))
                for anno_i, anno in enumerate(gt_dict[str_im_id]):
                    obj_id = anno["obj_id"]
                    if obj_id not in id2obj:
                        continue
                    ...
                    if mask.sum() == 0:  # NOTE: this should be ignored at training phase
                        print(
                            f"not visible, split {split} scene {scene_id}, im {int_im_id} obj {id2obj[obj_id]} {obj_id}"
                        )
                        print(f"{save_path}")
                        xyz_info = {
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="gen tless train_pbr xyz")
    ...
    setproctitle.setproctitle(
        f"gen_xyz_tless_train_pbr_{args.split}_{args.scene}")
