...
data_dir = osp.normpath(
    osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/itodd/train_pbr"))

cls_indexes = sorted(id2obj.keys())
cls_names = [id2obj[cls_idx] for cls_idx in cls_indexes]
itodd_model_dir = osp.normpath(
    osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/itodd/models"))
model_paths = [
    osp.join(itodd_model_dir, f"obj_{obj_id:06d}.ply") for obj_id in cls_indexes]
texture_paths = None

scenes = [i for i in range(20, 49 + 1)]
xyz_root = osp.normpath(
    osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/itodd/train_pbr/xyz_crop"))

K = np.array(
    [[2992.63, 0.0, 633.886], [0.0, 3003.99, 489.554], [0, 0, 1]])
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="gen itodd train_pbr xyz")
    ...
    setproctitle.setproctitle(
        f"gen_xyz_itodd_train_pbr_{args.split}_{args.scene}")
