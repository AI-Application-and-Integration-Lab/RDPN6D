...
class TUDL_REAL_Dataset:
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg.get("dataset_root", osp.join(DATASETS_ROOT, "BOP_DATASETS/tudl/train_real"))
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.tudl_full.id2obj.items() if obj_name in self.objs]
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(self.models_root, f"obj_{ref.tudl_full.obj2id[obj_name]:06d}.ply"),
...
SPLITS_TUDL_REAL = dict(
    tudl_train_pbr=dict(
        ...
        xyz_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/tudl/train_real/xyz_crop"),
...
for obj in TUDL_OBJECTS:
    for split in ["train", "test"]:
        name = "tudl_real_{}_{}".format(obj, split)
        if split in ["train"]:
            filter_invalid = True
        elif split in ["test"]:
            filter_invalid = False
        ...
        if name not in SPLITS_TUDL_REAL:
            if split == "test":
                SPLITS_TUDL_REAL[name] = dict(
                name=name,
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_TUDL_REAL:
        used_cfg = SPLITS_TUDL_REAL[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, TUDL_REAL_Dataset(used_cfg))
...
def get_available_datasets():
    return list(SPLITS_TUDL_REAL.keys())
...
def load_pointcloud():
    ...
    for i in range(1,4):
        ...
        pcd = o3d.io.read_point_cloud(f'{DATASETS_ROOT}/BOP_DATASETS/tudl/models/obj_0000{i:02d}.ply')
...
def test_vis():
    import open3d as o3d
    #dset_name = 'tudl_train_pbr'
    dset_name = 'tudl_real_can_test'
    ...
    for d in dicts:
        ...
        kpts_3d_list = [models[anno["category_id"]] for anno in annos]
...
if __name__ == "__main__":
    ...
    setup_logger()
    register_with_name_cfg('tudl_real_can_test')
