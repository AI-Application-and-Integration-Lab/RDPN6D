...
class ICBIN_PBR_Dataset:
    def __init__(self, data_cfg):
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.icbin_full.id2obj.items() if obj_name in self.objs]
        ...
        if 'train' in self.name and 'pbr' in self.name:
            ...
            self.scenes = [f"{i:06d}" for i in [1,2,3]]
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(self.models_root, f"obj_{ref.icbin_full.obj2id[obj_name]:06d}.ply"),
...
ICBIN_OBJECTS = ref.icbin_full.objects
...
SPLITS_ICBIN_PBR = dict(
    icbin_train_pbr=dict(
        ...
        objs=ICBIN_OBJECTS,  # selected objects
...
# single obj splits
for obj in ref.icbin_full.objects:
        ...
        if split in ["train"]:
            ...
            if name not in SPLITS_ICBIN_PBR:
                SPLITS_ICBIN_PBR[name] = dict(
                    ...
                    models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/icbin/models_eval"),
                    xyz_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/icbin/train_pbr/xyz_crop"),
...
def load_pointcloud():
    ...
    for i in range(1,3):
        ...
        pcd = o3d.io.read_point_cloud(f'{DATASETS_ROOT}/BOP_DATASETS/icbin/models/obj_0000{i:02d}.ply')
...
def test_vis():
    import open3d as o3d
    dset_name = 'icbin_pbr_coffee_cup_train'
...
if __name__ == "__main__":
    ...
    setup_logger()
    register_with_name_cfg( 'icbin_pbr_coffee_cup_train')
