...
class HB_PBR_Dataset:
    def __init__(self, data_cfg):
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.hb_full.id2obj.items() if obj_name in self.objs]
        ...
        if 'train' in self.name and 'pbr' in self.name:
            ...
            self.scenes = [f"{i:06d}" for i in [3,5,13]]
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(self.models_root, f"obj_{ref.hb_full.obj2id[obj_name]:06d}.ply"),
...
HB_OBJECTS = ref.hb_full.objects
...
# single obj splits
for obj in ref.hb_full.objects:
        ...
        if split in ["train"]:
            ...
            if name not in SPLITS_HB_PBR:
                SPLITS_HB_PBR[name] = dict(
                    ...
                    models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/hb/models_eval"),
                    xyz_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/hb/train_pbr/xyz_crop"),
...
def load_pointcloud():
    ...
    for i in range(1,31):
        ...
        pcd = o3d.io.read_point_cloud(f'{DATASETS_ROOT}/BOP_DATASETS/hb/models/obj_0000{i:02d}.ply')
...
def test_vis():
    ...
    for d in dicts:
        ...
        depth = mmcv.imread("/media/sda1/r10922190/RDPN-main/" + d["depth_file"], "unchanged") / d["depth_factor"]
...
if __name__ == "__main__":
    ...
    setup_logger()
    register_with_name_cfg( 'hb_pbr_01_bear_test')
