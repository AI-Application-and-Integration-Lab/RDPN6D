...
def crop_resize_by_d2_roialign(
    ...
    rois = torch.tensor(np.array(
        [0] + [cx - bw / 2, cy - bh / 2, cx + bw / 2, cy + bh / 2], dtype="float32")[None])
...
def my_warp_affine(img, center, scale, output_size, rot=0, interpolation=cv2.INTER_LINEAR):
    """
    output_size: int or (w, h)
    NOTE: if img is (h,w,1), the output will be (h,w)
    """
...
def xyz_to_region(xyz_crop, fps_points):
    ...
    mask_crop = ((xyz_crop[:, :, 0] != 0) | (xyz_crop[:, :, 1] != 0) | (
        xyz_crop[:, :, 2] != 0)).astype("uint8")
    dists = cdist(xyz_crop.reshape(bh * bw, 3), fps_points)  # (hw, f)
    region_ids = np.argmin(dists, axis=1).reshape(
        bh, bw) + 1  # NOTE: 1 to num_fps

    # * new
    def region_id_to_fps(x): return fps_points[x - 1]
    region_fps = region_id_to_fps(region_ids)
    delta = xyz_crop - region_fps

    # (bh, bw)
    return mask_crop * region_ids, delta  # 0 means bg
